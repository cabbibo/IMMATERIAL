...
float3 refl = reflect( normalize(_WorldSpaceCameraPos-v.worldPos) , v.nor);
        float reflM = dot( refl , _WorldSpaceLightPos0 );
                ...
                float4 tCol = 1-tex2D(_MainTex, v.uv.yx * float2( 1  ,1 ) ) * 3;
                ...
                float4 cCol = texCUBE(_CubeMap,refl);;// * color;

                float4 cmCol = tex2D( _ColorMap , float2( tCol.x * .1 + .8 + val * .1 , 0) ) + float4(0,0,.6* v.uv.x,1);

                //if( sin( v.uv.x  * 100 ) > .5 ){discard;}
               // if( ( lookupVal + 1.3) - 1.2*length( tCol ) < .5 ){ discard;}
                fixed4 col = tCol  * cmCol * cCol * 2;//float4( cCol , 1 ) * 2 * shadow * v.uv.x * 1.4 * tex2D(_ColorMap , float2( -length(tCol) * .2 * v.uv.x * v.uv.x*v.uv.x* .5 - val * .1  + sin( v.kelpID) * .02 +  _HueStart, 0) );// * saturate(20*-val);//* 20-10;//*tCol* lookupVal*4;//* 10 - 1;
